...
int main(int argc, char** argv)
{
  ...
  std::map<std::string, double> ipopt_options;
  ipopt_options["tol"] = 10;
  ipopt_options["dual_inf_tol"] = 5;
  ipopt_options["compl_inf_tol"] = 0.01;
...
//  ipopt.set_integer_option("max_iter", 1);
  ...
  pagmo::population pop(pagmo_prob, 0);
  pop.push_back(initial_vec);
  ...
  Eigen::MatrixXd traj = trajopt::getTraj(pop.champion_x(), prob->GetVars());
  std::cout << traj << "\n";
